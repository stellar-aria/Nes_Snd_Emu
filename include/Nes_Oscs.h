...
struct Nes_Square : Nes_Envelope {
  ...
  using Synth = Blip_Synth_Norm;
  Synth const& synth;  // shared between squares
  const int min_period;

  Nes_Square(Synth const* s, int minimumPeriod = 8) : Nes_Envelope(), synth(*s), min_period(minimumPeriod) {
  }

  void clock_sweep(int adjust);
  void run(nes_time_t /*time*/, nes_time_t /*end_time*/);
  void reset() {
    sweep_delay = 0;
    Nes_Envelope::reset();
  }
...
struct Nes_Dmc : Nes_Osc {
  ...
  void reload_sample();
  void reset();
  int count_reads(nes_time_t /*time*/, nes_time_t* /*last_read*/) const;
  [[nodiscard]] nes_time_t next_read_time() const;
};
