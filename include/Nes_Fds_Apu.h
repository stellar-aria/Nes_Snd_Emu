...
class Nes_Fds_Apu {
 public:
  ...
  void set_output(int index,
                  Blip_Buffer* center,
                  Blip_Buffer* left_ignored = nullptr,
                  Blip_Buffer* right_ignored = nullptr);
 ...
 private:
  ...
  int lfo_tempo{lfo_base_tempo};  // normally 8; adjusted by set_tempo()
...
#pragma warning(pop)
  ...
  uint8_t& regs(unsigned addr) {
    return regs_[addr - io_addr];
  }
...
inline void Nes_Fds_Apu::set_output(int i, Blip_Buffer* buf, Blip_Buffer* /*unused*/, Blip_Buffer* /*unused*/) {
