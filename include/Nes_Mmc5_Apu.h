...
class Nes_Mmc5_Apu {
 public:
  void set_output(Blip_Buffer* /*b*/);

  enum { regs_addr = 0x5000 };
  enum { regs_size = 0x16 };

  void write_register(blip_time_t /*time*/, uint16_t addr, uint8_t data);
  ...
  uint8_t read_status(blip_time_t /*time*/);
  ...
  // the IRQ flag.
  uint8_t read_irq_status(blip_time_t /*time*/);
...
#ifdef _MSC_VER
#pragma warning(push)
  // prevents "warning C4251: 'Nes_Mmc5_Apu::irq_notifier': class 'std::function<void (bool)>' needs to have
  // dll-interface to be used by clients of class 'Nes_Mmc5_Apu'" std::function<>, being a templated class inside of the
  // STL, is guaranteed to generate code where needed for clients, because it's all in the header
...
#pragma warning(pop)
  ...
  void state_restored();
  void run_until_(blip_time_t /*end_time*/);
};
