...
class Nes_Apu {
 public:
  ...
  using nes_time_t = int;  // NES CPU clock cycle count

  // Sets buffer to generate sound into, or 0 to mute output (reduces
  // emulation accuracy).
  void set_output(Blip_Buffer* /*buffer*/);
  ...
  enum { io_addr = 0x4000 };
  enum { io_size = 0x18 };
  void write_register(nes_time_t /*time*/, uint16_t addr, uint8_t data);

  // Reads from status register (0x4015)
  enum { status_addr = 0x4015 };
  uint8_t read_status(nes_time_t /*time*/);
  ...
  enum { irq_waiting = 0 };
  [[nodiscard]] nes_time_t earliest_irq(nes_time_t /*unused*/) const;
  ...
  [[nodiscard]] nes_time_t next_dmc_read_time() const;
...
#ifdef _MSC_VER
#pragma warning(push)
  // prevents "warning C4251: 'Nes_Apu::dmc_reader': class 'std::function<int (int)>' needs to have dll-interface to be
  // used by clients of class 'Nes_Apu'" std::function<>, being a templated class inside of the STL, is guaranteed to
  // generate code where needed for clients, because it's all in the header
...
#pragma warning(pop)
  ...
  static float tnd_total_() {
    return 196.015f;
  }

  void enable_w4011_(bool enable = true) {
    enable_w4011 = enable;
  }
...
#pragma warning(pop)
  ...
  void irq_changed();
  void state_restored();
  void run_until_(nes_time_t /*end_time*/);
};
...
inline Nes_Apu::nes_time_t Nes_Apu::earliest_irq(nes_time_t /*unused*/) const {
...
inline Nes_Apu::nes_time_t Nes_Apu::next_dmc_read_time() const {
  return dmc.next_read_time();
}
