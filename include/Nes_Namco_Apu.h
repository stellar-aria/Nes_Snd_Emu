...
#pragma warning(pop)
  ...
  uint8_t& access();
  void run_until(blip_time_t /*nes_end_time*/);
};
/*
struct namco_state_t
{
...
inline void Nes_Namco_Apu::volume(double v) {
  synth.volume(0.10 / osc_count / 15 * v);
}

inline void Nes_Namco_Apu::treble_eq(const blip_eq_t& eq) {
  synth.treble_eq(eq);
}

inline void Nes_Namco_Apu::write_addr(uint8_t v) {
  addr_reg = v;
}

inline uint8_t Nes_Namco_Apu::read_data() {
  return access();
}
