...
Silent_Buffer::Silent_Buffer()
    : Multi_Buffer(1)  // 0 channels would probably confuse
{
...
Mono_Buffer::~Mono_Buffer() = default;
...
Stereo_Buffer::~Stereo_Buffer() = default;
...
void Stereo_Mixer::read_pairs(blip_sample_t out[], int count) {
  ...
  samples_read += count;
  if ((bufs[0]->non_silent() | bufs[1]->non_silent()) != 0u) {
...
void Stereo_Mixer::mix_mono(blip_sample_t out_[], int count) {
  ...
  int offset = -count;
  do {
    int s = center_sum >> Tracked_Blip_Buffer::delta_bits;
