...
void Nes_Osc::clock_length(int halt_mask) {
  if ((length_counter != 0) && ((regs[0] & halt_mask) == 0)) {
    length_counter--;
  }
}
...
int Nes_Envelope::volume() const {
  return length_counter == 0 ? 0 : ((regs[0] & 0x10) != 0) ? (regs[0] & 15) : envelope;
}
...
// TODO: clean up
inline Nes_Square::nes_time_t Nes_Square::maintain_phase(nes_time_t time,
                                                         nes_time_t end_time,
...
// TODO: clean up
inline Nes_Square::nes_time_t Nes_Triangle::maintain_phase(nes_time_t time,
                                                           nes_time_t end_time,
                                                           nes_time_t timer_period) {
  nes_time_t remain = end_time - time;
  if (remain > 0) {
...
void Nes_Dmc::recalc_irq() {
  ...
  if (irq_enabled && (length_counter != 0)) {
    irq = apu->last_dmc_time + delay + ((length_counter - 1) * 8 + bits_remain - 1) * nes_time_t(period) + 1;
  }
  if (irq != next_irq) {
    next_irq = irq;
    apu->irq_changed();
...
int Nes_Dmc::count_reads(nes_time_t time, nes_time_t* last_read) const {
  ...
  if (((regs[0] & loop_flag) == 0) && count > length_counter) {
...
static int const dmc_table[128] = {
    0,    24,   48,   71,   94,   118,  141,  163,  186,  209,  231,  253,  275,  297,  319,  340,  361,  383,  404,
    425,  445,  466,  486,  507,  527,  547,  567,  587,  606,  626,  645,  664,  683,  702,  721,  740,  758,  777,
    795,  813,  832,  850,  867,  885,  903,  920,  938,  955,  972,  989,  1006, 1023, 1040, 1056, 1073, 1089, 1105,
    1122, 1138, 1154, 1170, 1185, 1201, 1217, 1232, 1248, 1263, 1278, 1293, 1308, 1323, 1338, 1353, 1368, 1382, 1397,
    1411, 1425, 1440, 1454, 1468, 1482, 1496, 1510, 1523, 1537, 1551, 1564, 1578, 1591, 1604, 1618, 1631, 1644, 1657,
    1670, 1683, 1695, 1708, 1721, 1733, 1746, 1758, 1771, 1783, 1795, 1807, 1819, 1831, 1843, 1855, 1867, 1879, 1890,
    1902, 1914, 1925, 1937, 1948, 1959, 1971, 1982, 1993, 2004, 2015, 2026, 2037, 2048,
};
...
static short const noise_period_table[16] = {0x004, 0x008, 0x010, 0x020, 0x040, 0x060, 0x080, 0x0A0,
...
void Nes_Noise::run(nes_time_t time, nes_time_t end_time) {
  ...
  if (time < end_time) {
    ...
    if (volume == 0) {
      ...
      int delta = amp * 2 - volume;
      const int tap = (((regs[2] & mode_flag) != 0) ? 8 : 13);
      output->set_modified();
