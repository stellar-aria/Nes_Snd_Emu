...
void Nes_Fds_Apu::run_until(blip_time_t final_end_time) {
  ...
  if ((wave_freq != 0) && (output_ != nullptr) && (((regs(0x4089) | regs(0x4083)) & 0x80) == 0)) {
    output_->set_modified();

// master_volume
#define MVOL_ENTRY(percent) ((master_vol_max * (percent) + 50) / 100)
    static unsigned char const master_volumes[4] = {MVOL_ENTRY(100), MVOL_ENTRY(67), MVOL_ENTRY(50), MVOL_ENTRY(40)};
    int const master_volume = master_volumes[regs(0x4089) & 0x03];

    // lfo_period
    blip_time_t lfo_period = regs(0x408A) * lfo_tempo;
    if ((regs(0x4083) & 0x40) != 0) {
      lfo_period = 0;
    }
    ...
    if ((sweep_period == 0) || ((regs(0x4084) & 0x80) != 0)) {
    ...
    if ((env_period == 0) || ((regs(0x4080) & 0x80) != 0)) {
    ...
    do {
      ...
      if (end_time > env_time) {
        end_time = env_time;
      }
      if (end_time > sweep_time) {
        end_time = sweep_time;
      }
